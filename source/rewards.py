...
def penalize_static_arms(env: ManagerBasedRLEnv,asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),arm_joint_keywords: list = ["shoulder", "elbow", "wrist"],min_movement: float = 0.05,penalty_weight: float = -1.0) -> torch.Tensor:
    """
    懲罰手臂關節靜止（速度低於 min_movement）。
    """
    asset: Articulation = env.scene[asset_cfg.name]
    joint_names = asset.joint_names
    ...
    for i, name in enumerate(joint_names):
        name_lower = name.lower()
        if any(keyword in name_lower for keyword in arm_joint_keywords):
    ...
    # 取得手臂關節速度
    arm_vel = joint_vel[:, arm_joint_indices]  # [num_envs, num_arm_joints]
    abs_vel = torch.abs(arm_vel)

    # 判斷哪些關節速度低於閾值
    static_mask = (abs_vel < min_movement).float()
    # 每個環境有多少手臂關節沒在動
    static_count = torch.sum(static_mask, dim=1)
    # 懲罰分數
    penalty = penalty_weight * static_count
    return penalty